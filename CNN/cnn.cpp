...
void cnntrain(CNN* cnn,	ImgArr inputData,LabelArr outputData,CNNOpts opts,int trainNum)
{
	...
	for(e=0;e<opts.numepochs;e++){
		...
		for(n=0;n<trainNum;n++){
			...
			/*char* filedir="C:\\Users\\cuteandbeautifulhong\\Desktop\\DeepLearningC-master\\DeepLearningC-master\\CNN\\Minst\\CNNData.txt";
...
void cnnff(CNN* cnn,float** inputData)
{
	...
	FILE *fp = fopen("C:\\just a test\\test.txt", "at+");
	printf("%f  \n", cnn->C1->y[1][1][1] + cnn->C1->basicData[1]);
	fwrite(&(cnn->C1->y[1][1][1]), sizeof(cnn->C1->y[1][1][1]), 1, fp);
	printf("%f  \n", cnn->C1->y[1][1][1]);
	fclose(fp);
